...
### Recipe flags and tags
- **Flags** are three toggle buttons in the Recipe Builder, next to the Tags button, that change what a recipe does:
  - `⊘` **Not counted by safeguards** — the recipe still keeps its gear, but Safeguards no longer count those keeps as coverage. Use it for "kept for rework fodder" recipes that should not satisfy a Safeguard quota.
  - `⟳` **Only reworked gear** — the recipe applies **only** to gear that has been reworked (a Chaos Ore was applied to it).
  - `⊙` **Only not-reworked gear** — the recipe applies **only** to gear that has not been reworked.
- The two rework buttons are mutually exclusive: turning one on turns the other off, and turning the active one off means the recipe applies to all gear regardless of rework state. Each active flag also shows as a small glyph on the recipe in the list below the builder.
- **Tags** are optional keywords you attach to a recipe. On their own they do nothing — they are labels for your own organization. What makes them useful is that a recipe file's note can target them: the Dynamic Overrides in the bundled presets pick recipes by tag to switch whole groups on or off from the questions shown after loading. So tagging a recipe is how you make it selectable by those options.
- Older recipe files used special tags (`ignore_by_safeguards`, `if_reworked`, `if_not_reworked`) for the three flags above. Those files still load: the tags are converted into the matching flags automatically.
- When any rule is limited this way, the preview table shows an extra **Ore** column so you can see which rows are restricted.
...
- `Weight by Rank` (the chip next to `Per Mainstat`) folds rank into the score itself instead of leaving it as an exact tie-break. The bonus is scaled by the **main stat**, and the exact size depends on the slot and main stat — a 5★ main stat scores roughly 81–89% of the same slot's 6★, a 4★ less again. The gap between adjacent ranks is small: enough to let a 6★ win a *near*-tie against a same-main-stat 5★ with only slightly better substats, but not enough to beat a real substat advantage. It uses the rank's +16 main stat, so leveling a piece never changes its score, and substat scores are untouched. Needs at least one substat selected; new rules have it on by default.
  - The tune button on the `Weight by Rank` chip opens the weights flyout, where the main stat appears as its own weighted row (`Mainstat (by rank)`) alongside the substats. Its weight is a multiplier on the rank bonus above: leave it at `1.0` for the default behavior, raise it to make a higher-rank piece's main stat outweigh a lower-rank piece's better substats, or drop it toward `0` to lean fully on substats. This is how you decide whether rank or substats win when the two disagree — the candidate list re-ranks live as you adjust it.
- `≥ Required Count` is the promise you want the app to keep. Scope chips directly under each picker (everything except `Rank` and `Rarity`) decide how that quota is split: `Per slot`, `Per set`, `Per faction`, and `Per Mainstat` can be combined freely. Turning every chip off automatically falls back to the shared `Per safeguard` pool.
...
Stacking the scope chips changes who gets their own quota. Leave `Per safeguard` on to share one pool, then layer `Per slot`, `Per set`, `Per faction`, and `Per Mainstat` to enforce Slot × Set × Faction × Main stat mixes independently.
...
| `Per Mainstat` | Duplicates the quota for each selected main stat. | Keep separate pools for ACC versus RES banners. |

#### Groups: several safeguards sharing one Required Count
Two safeguards can end up describing the *same* promise without meaning to. One keeps 3 good rings, another keeps 2 — but you only ever wanted 3 rings in total, and now you are keeping 5.

A **group** fixes that. Select the safeguards (click, then Ctrl-click) and pick `Create Group` from the right-click menu. Every member now shows a coloured edge in the list, and a small group strip appears right beside its `Required Count` in the form: the group's own count and its own scope buttons.

- Every safeguard in a group still looks for **exactly** what it always did — its own slots, sets, substats, weights and mode. A group never changes what they search for.
- What changes is where their finds go. A safeguard in a group no longer keeps gear on its own: it **offers** its finds to the group, and its own `Required Count` decides how many it offers.
- The group's count (the `⊚ ≥` field next to `Required Count`) is the number that actually decides. Type it straight in — that is how much the group keeps out of everything its safeguards offered it. It starts as the smallest of the members' counts. Editing it on any member edits the group.
- The group's own scope buttons decide what counts as the same pool. Put `faction` on the group and each faction gets its own count, however the safeguards inside are split up.
- When several safeguards offer pieces for the same pool, the **best pieces win**. Each piece is judged by the substats and weights of the safeguard that offered it, so a piece from the second safeguard can absolutely beat one from the first. The candidates flyout marks pieces the group sold in the group's colour, and its `Group` toggle shows the whole pool ranked together.
- Groups are contiguous: drag a safeguard into a group (drop it inside the coloured bracket) or out of it (drop it flush), or use the right-click menu — `Remove from Group`, `Join Group Above/Below`, `Ungroup`. Moving a safeguard up or down never changes its group; a whole group moves as one block.

**Example.** You want five good SPD accessories per faction, but "good" means two different things: `SPD + CR + CDMG` for damage dealers, `SPD + HP + DEF` for tanks. Make one safeguard for each profile, group them, and set the group to `5` with `faction`. Each safeguard offers its best pieces, and the group keeps the five that came out best between them — instead of five of each.
...
| Core accessories per faction | Three rules (Ring, Amulet, Banner), Sets/Main blank | `5` | `Per faction` | Five Rings, five Amulets and five Banners survive per faction — each rule keeps its own five. |
| Five accessories per faction, any slot | The same three rules, in one group | `5` as the group's count | `faction` on the group | Five accessories in total survive per faction, whichever slots they happen to be. |
